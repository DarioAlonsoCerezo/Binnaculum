...
module FileProcessor =
    ...
    let processFile (filePath: string) =
        ...
        | ".zip" ->
            ...
            { FilePath = tempDir
              IsTemporary = true
              CsvFiles = csvFiles }
        | _ -> failwith $"Unsupported file format: {extension}. Only CSV and ZIP files are supported."
    ...
    let validateCsvFiles (processedFile: ProcessedFile) =
        ...
        |> List.forall (fun csvFile -> File.Exists(csvFile) && File.ReadAllLines(csvFile).Length > 0)
