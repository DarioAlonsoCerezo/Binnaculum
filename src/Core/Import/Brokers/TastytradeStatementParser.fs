...
module TastytradeStatementParser =
    ...
    let private parseDecimal (value: string) : decimal =
        if String.IsNullOrWhiteSpace(value) || value = "--" then
            ...
            with _ ->
                failwith $"Invalid decimal value: '{value}'"
    ...
    let private parseDate (dateStr: string) : DateTime =
        ...
        with ex ->
            failwith
                $"Invalid date format: '{dateStr}'. Expected ISO format like '2024-05-31T14:42:13+0100'. Error: {ex.Message}"
    ...
    let private parseExpirationDate (dateStr: string) : DateTime option =
        if String.IsNullOrWhiteSpace(dateStr) then
            ...
            with ex ->
                let errorMsg =
                    sprintf
                        "Invalid expiration date format: '%s'. Expected format like '5/31/24'. Error: %s"
                        dateStr
                        ex.Message
    ...
    let private splitCsvLine (line: string) : string array =
        ...
        while i < line.Length do
            ...
            | ',' when not insideQuotes ->
                ...
                currentField <- ""
            | _ -> currentField <- currentField + string char
        ...
        fields |> List.rev |> List.map (fun f -> f.Trim().Trim('"')) |> List.toArray
    ...
    /// </summary>
    let private parseTransactionLine
        (fields: string array)
        (lineNumber: int)
        (rawLine: string)
        (hasTotal: int)
                ...
                { LineNumber = lineNumber
                  ...
                  ErrorType =
                    if ex.Message.Contains("date") then
                        InvalidDateFormat
                    elif ex.Message.Contains("decimal") || ex.Message.Contains("numeric") then
                        InvalidNumericValue(ex.Message)
                    elif ex.Message.Contains("transaction type") then
                        InvalidTransactionType
                    else
                        TastytradeErrorType.InvalidNumericValue(ex.Message) }
