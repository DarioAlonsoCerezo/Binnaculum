...
module IBKRStatementParser =
            ...
            | Some dt, Some qty, Some proc, Some comm ->
                Some
                    { AssetCategory = fields.[2]
                      ...
                      RealizedPnLPercent =
                        if fields.Length > 12 then
                            parseDecimal fields.[12]
                        else
                            None
                      MTMPnL =
                        if fields.Length > 13 then
                            parseDecimal fields.[13]
                        else
                            None
    ...
    let private parseCashMovement (fields: string[]) : IBKRCashMovement option =
        if fields.Length < 5 then
            ...
            let movementType =
                if description.Contains("Electronic Fund Transfer") then
                    Deposit
                elif description.Contains("Withdrawal") then
                    Withdrawal
                elif description.Contains("Commission") then
                    Commission
                else
                    TradeSettlement
    ...
    let private parseExchangeRate (fields: string[]) : IBKRExchangeRate option =
        if fields.Length < 4 then
            ...
            match rate with
            | Some r -> Some { Currency = currency; Rate = r }
            | _ -> None

    /// <summary>
    /// Parse lines for a specific section
    /// </summary>
    let private parseSection
        (section: IBKRSection)
        (lines: string list)
        (data: IBKRStatementData)
        : IBKRStatementData * string list =
        ...
        | Trades ->
            ...
            ({ data with
                ...
                ForexTrades = data.ForexTrades @ forexTrades },
             errors |> List.ofSeq)
        ...
        | DepositsWithdrawals ->
            ...
            ({ data with
                CashMovements = data.CashMovements @ movements },
             errors |> List.ofSeq)
        ...
        | CashReport ->
            ...
            ({ data with
                CashFlows = data.CashFlows @ flows },
             errors |> List.ofSeq)
        ...
        | OpenPositions ->
            ...
            ({ data with
                OpenPositions = data.OpenPositions @ positions },
             errors |> List.ofSeq)
        ...
        | FinancialInstruments ->
            ...
            ({ data with
                Instruments = data.Instruments @ instruments },
             errors |> List.ofSeq)
    ...
    let parseCsvContent (content: string) : IBKRParseResult =
        ...
        let result = parseCsvFile tempFile
