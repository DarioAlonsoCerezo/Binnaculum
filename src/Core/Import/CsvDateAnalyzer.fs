...
module CsvDateAnalyzer =
    ...
    let extractDateFromFilename (fileName: string) : DateTime option =
        try
            ...
            if ibkrMatch.Success then
                ...
                let mutable parsedDate = DateTime.MinValue
                if DateTime.TryParseExact(dateStr, "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None, &parsedDate) then
                    Some parsedDate
                else
                    None
            else
                ...
                if tastyMatch.Success then
                    ...
                    let mutable parsedDate = DateTime.MinValue
                    if DateTime.TryParseExact(dateStr, "yyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None, &parsedDate) then
                        Some parsedDate
                    else
                        None
                else
        ...
        if String.IsNullOrWhiteSpace(csvRow) then
            ...
            try
                ...
                |> Array.tryPick (fun field ->
                    let mutable parsedDate = DateTime.MinValue
                    if DateTime.TryParseExact(field, dateFormats, CultureInfo.InvariantCulture, DateTimeStyles.None, &parsedDate) && parsedDate.Year > 2000 then
                        Some parsedDate
    ...
    let extractAllDatesFromCsv (filePath: string) : DateTime list =
        ...
        with
        | ex ->
            CoreLogger.logWarningf "CsvDateAnalyzer" 
                "Failed to scan %s: %s" (Path.GetFileName filePath) ex.Message
            []
    ...
    let analyzeFile (filePath: string) : CsvFileMetadata =
        ...
        let exactRowCount = 
            try
                File.ReadAllLines(filePath).Length - 1  // Minus header
            with
            | _ -> 0
            ...
            |> List.collect (fun (prev, next) ->
                ...
                | Some prevEnd, Some nextStart ->
                    ...
                    if prevEnd >= nextStart then
                        ...
                        overlappingDates
                        |> List.map (fun date -> (prev.FileName, next.FileName, date))
                    else
                        []
                | _ -> []
            )
