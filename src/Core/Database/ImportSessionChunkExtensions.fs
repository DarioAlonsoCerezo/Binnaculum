...
type Do() =
    ...
    static member createChunks(sessionId: int, chunks: ChunkInfo list) = task {
        ...
        for chunk in chunks do
            ...
            command.Parameters.AddWithValue("@StartDate", chunk.StartDate.ToString("O", System.Globalization.CultureInfo.InvariantCulture)) |> ignore
            command.Parameters.AddWithValue("@EndDate", chunk.EndDate.ToString("O", System.Globalization.CultureInfo.InvariantCulture)) |> ignore
            command.Parameters.AddWithValue("@EstimatedMovements", chunk.EstimatedMovements) |> ignore
    ...
    static member getBySessionId(sessionId: int) = task {
        let! command = Database.Do.createCommand()
    ...
    static member getPendingChunks(sessionId: int) = task {
        let! command = Database.Do.createCommand()
    ...
    static member markInProgress(chunkId: int) = task {
        let! command = Database.Do.createCommand()
    ...
    static member markFailed(chunkId: int, errorMessage: string) = task {
        let! command = Database.Do.createCommand()
    ...
    static member getBySessionAndChunkNumber(sessionId: int, chunkNumber: int) = task {
        let! command = Database.Do.createCommand()
