...
type Do() =
    ...
    static member getAll() =
        Database.Do.getAllEntities Do.read DividendsQuery.getAll

    static member getById(id: int) =
        Database.Do.getById Do.read id DividendsQuery.getById
...
type DividendCalculations() =
    ...
    [<Extension>]
    static member calculateDividendCount(dividends: Dividend list) = dividends.Length
    ...
    static member filterByCurrency(dividends: Dividend list, currencyId: int) =
        dividends |> List.filter (fun dividend -> dividend.CurrencyId = currencyId)
    ...
    static member filterByTicker(dividends: Dividend list, tickerId: int) =
        dividends |> List.filter (fun dividend -> dividend.TickerId = tickerId)
    ...
    static member getUniqueCurrencyIds(dividends: Dividend list) =
        dividends |> List.map (fun dividend -> dividend.CurrencyId) |> Set.ofList
    ...
    static member getUniqueTickerIds(dividends: Dividend list) =
        dividends |> List.map (fun dividend -> dividend.TickerId) |> Set.ofList
    ...
    [<Extension>]
    static member calculateDividendIncomeInDateRange
        (dividends: Dividend list, startDate: DateTimePattern, endDate: DateTimePattern)
        =
        dividends
        |> List.filter (fun dividend ->
            dividend.TimeStamp.Value >= startDate.Value
            && dividend.TimeStamp.Value <= endDate.Value)
        |> List.sumBy (fun dividend -> dividend.DividendAmount.Value)
        |> Money.FromAmount
