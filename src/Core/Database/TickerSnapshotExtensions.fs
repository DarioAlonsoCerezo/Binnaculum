...
type Do() =
    ...
    [<Extension>]
    static member save(tickerSnapshot: TickerSnapshot) =
        Database.Do.saveEntity tickerSnapshot (fun t c -> t.fill c)

    [<Extension>]
    static member delete(tickerSnapshot: TickerSnapshot) = Database.Do.deleteEntity tickerSnapshot

    static member getAll() =
        Database.Do.getAllEntities Do.read TickerSnapshotQuery.getAll

    static member getById(id: int) =
        Database.Do.getById Do.read id TickerSnapshotQuery.getById
    ...
    static member getTickerSnapshotsByDateRange
        (
            tickerId: int,
            startDate: Binnaculum.Core.Patterns.DateTimePattern,
            endDate: Binnaculum.Core.Patterns.DateTimePattern
        ) =
        task {
            let! command = Database.Do.createCommand ()
    ...
    static member getTickerSnapshotsAfterDate(tickerId: int, date: Binnaculum.Core.Patterns.DateTimePattern) =
        task {
            let! command = Database.Do.createCommand ()
            command.CommandText <- TickerSnapshotQuery.getTickerSnapshotsAfterDate
            command.Parameters.AddWithValue(SQLParameterName.TickerId, tickerId) |> ignore
