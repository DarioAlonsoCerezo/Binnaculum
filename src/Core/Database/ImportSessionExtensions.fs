...
type Do() =
    ...
    static member createSession(
        ...
        command.Parameters.AddWithValue("@BrokerAccountName", brokerAccountName) |> ignore
        ...
        command.Parameters.AddWithValue("@State", "Phase1_PersistingMovements") |> ignore
        command.Parameters.AddWithValue("@Phase", "Phase1_PersistingMovements") |> ignore
        ...
        command.Parameters.AddWithValue("@MinDate", minDate.ToString("yyyy-MM-dd HH:mm:ss")) |> ignore
        command.Parameters.AddWithValue("@MaxDate", maxDate.ToString("yyyy-MM-dd HH:mm:ss")) |> ignore
        command.Parameters.AddWithValue("@TotalEstimatedMovements", totalEstimatedMovements) |> ignore
        command.Parameters.AddWithValue("@StartedAt", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")) |> ignore
        command.Parameters.AddWithValue("@LastProgressUpdateAt", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")) |> ignore
    ...
    static member getActiveSession(brokerAccountId: int) = task {
        let! command = Database.Do.createCommand()
    ...
    /// Get session by ID
    static member getById(sessionId: int) = task {
        let! command = Database.Do.createCommand()
    ...
    static member updatePhase(sessionId: int, phase: string) = task {
        let! command = Database.Do.createCommand()
    ...
    static member markPhase1Completed(sessionId: int) = task {
        let! command = Database.Do.createCommand()
    ...
    /// Update chunk progress - MUST be called within a transaction
    static member updateChunkProgress(sessionId: int, movementsCount: int, transaction: SqliteTransaction) = task {
    ...
    static member markBrokerSnapshotsCompleted(sessionId: int) = task {
        let! command = Database.Do.createCommand()
    ...
    static member markTickerSnapshotsCompleted(sessionId: int) = task {
        let! command = Database.Do.createCommand()
    ...
    static member completeSession(sessionId: int) = task {
        let! command = Database.Do.createCommand()
    ...
    static member markFailed(sessionId: int, errorMessage: string) = task {
        let! command = Database.Do.createCommand()
    ...
    static member markCancelled(sessionId: int) = task {
        let! command = Database.Do.createCommand()
