...
type Do() =
    ...
    static member save(ticker: Ticker) =
        Database.Do.saveEntity ticker (fun t c -> t.fill c)

    [<Extension>]
    static member delete(ticker: Ticker) = Database.Do.deleteEntity ticker

    static member getAll() =
        Database.Do.getAllEntities Do.read TickersQuery.getAll

    static member getById(id: int) =
        Database.Do.getById Do.read id TickersQuery.getById

    static member getBySymbol(symbol: string) : System.Threading.Tasks.Task<Ticker> =
        task {
            ...
            match ticker with
            | Some existingTicker -> return existingTicker
            | None ->
                ...
                // Try to get icon from repository before creating ticker
                let! icon = Binnaculum.Core.Providers.TickerIconProvider.getTickerIcon (symbol)
    ...
    static member insertIfNotExists() =
        task {
            let audit =
                { CreatedAt = Some(DateTimePattern.FromDateTime(DateTime.Now))
                  UpdatedAt = None }
