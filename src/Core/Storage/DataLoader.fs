...
module internal DataLoader =
    ...
    let internal loadAddedBank () =
        task {
            ...
            if not hasDefaultBank then
                Collections.Banks.Add(
                    { Id = -1
                      Name = "AccountCreator_Create_Bank"
                      Image = Some "bank"
    ...
    let internal loadAddedBroker () =
        task {
            ...
            if not hasDefaultBroker then
                Collections.Brokers.Add(
                    { Id = -1
                      Name = "AccountCreator_Create_Broker"
                      Image = "broker"
                      SupportedBroker = SupportedBroker.Unknown }
                )
        }

    let getOrRefresAvailableImages () =
        task {
            let directory = AppDataDirectoryProvider.getAppDataDirectory ()
    ...
    let private loadBrokerAccounts () =
        task {
            ...
            let brokerAccounts =
                ...
                |> List.map (fun account ->
                    async {
                        ...
                        let! hasMovements = BrokerAccountExtensions.Do.hasMovements account.Id |> Async.AwaitTask
    ...
    let getOrRefreshAllAccounts () =
        task {
            ...
            if allAccounts.IsEmpty then
                Collections.Accounts.Add(
                    { Type = AccountType.EmptyAccount
                      Broker = None
                      Bank = None
                      HasMovements = false }
                )
    ...
    let loadOverviewSnapshots () =
        task {
            ...
            do!
                DataLoader.BrokerAccountSnapshotLoader.load ()
                |> Async.AwaitTask
                |> Async.Ignore
    ...
    let private updateBankAccount (account: Account, movements: Movement list) =
        ...
        if hasMovements <> account.HasMovements then
            ...
            let current =
                ...
                |> Seq.find (fun a -> a.Bank.IsSome && a.Bank.Value.Id = account.Bank.Value.Id)
        ...
        task {
            ...
            |> Seq.iter (fun account ->
                if account.Bank.IsSome then
                    (account, movements) |> updateBankAccount

                if account.Broker.IsSome then
                    (account, movements) |> updateBrokerAccount)
        }
    ...
    let loadMovementsForBrokerAccount (brokerAccountId: int, startDate: DateTimePattern option) =
        task {
            let dateFilter =
                startDate
                |> Option.defaultValue (DateTimePattern.FromDateTime(DateTime.MinValue))

            // Use database-level filtering for optimal performance
            let! databaseBrokerMovements =
                BrokerMovementExtensions.Do.getByBrokerAccountIdFromDate (brokerAccountId, dateFilter)
            ...
            let! databaseDividendTaxes =
                DividendTaxExtensions.Do.getByBrokerAccountIdFromDate (brokerAccountId, dateFilter)
    ...
    let internal changeOptionsGrouped () =
        task {
            ...
            let movementsWithoutOptions =
                currentMovements |> List.filter (fun movement -> movement.OptionTrade.IsNone)
            ...
            |> Seq.iter (fun account ->
                if account.Broker.IsSome then
                    (account, updatedMovements) |> updateBrokerAccount)
        }
