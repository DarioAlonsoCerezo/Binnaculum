        ...
        static member brokerSnapshotToOverviewSnapshot(dbSnapshot: BrokerSnapshot, broker: Broker) =
            {
                ...
                Broker = Some {
                    ...
                    RealizedGains = dbSnapshot.Financial.RealizedGains.Value
                    RealizedPercentage = dbSnapshot.Financial.RealizedPercentage
                    UnrealizedGains = dbSnapshot.Financial.UnrealizedGains.Value
                    UnrealizedGainsPercentage = dbSnapshot.Financial.UnrealizedGainsPercentage
                    AccountCount = dbSnapshot.AccountCount
                    Invested = dbSnapshot.Financial.Invested.Value
                    Commissions = dbSnapshot.Financial.Commissions.Value
                    Fees = dbSnapshot.Financial.Fees.Value
                    Deposited = dbSnapshot.Financial.Deposited.Value
                    Withdrawn = dbSnapshot.Financial.Withdrawn.Value
                    DividendsReceived = dbSnapshot.Financial.DividendsReceived.Value
                    OptionsIncome = dbSnapshot.Financial.OptionsIncome.Value
                    OtherIncome = dbSnapshot.Financial.OtherIncome.Value
                    OpenTrades = dbSnapshot.Financial.OpenTrades
                }
                Bank = None
                BrokerAccount = None
                BankAccount = None
