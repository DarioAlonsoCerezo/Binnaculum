...
module internal SnapshotManager =
    ...
    let handleNewBankAccount (bankAccount: BankAccount) =
        task {
            ...
            do!
                BankSnapshotManager.updateBankSnapshot (
                    bankAccount.BankId,
                    DateTimePattern.FromDateTime(DateTime.Today)
                )
        }
    ...
    let handleNewBrokerAccount (brokerAccount: BrokerAccount) =
        task {
            ...
            do!
                BrokerSnapshotManager.updateBrokerSnapshot (
                    brokerAccount.BrokerId,
                    DateTimePattern.FromDateTime(DateTime.Today)
                )
        }
