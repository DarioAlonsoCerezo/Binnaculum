...
module internal Patterns =
    ...
    type DateTimePattern =
        private
        | DateTimePattern of DateTime

        /// Creates a DateTimePattern from a DateTime value
        static member FromDateTime(dateTime: DateTime) = DateTimePattern dateTime

        /// Parses a string into a DateTimePattern, ensuring it matches the ISO 8601 pattern
        /// Accepts both "yyyy-MM-ddTHH:mm:ss" (ISO 8601) and "yyyy-MM-dd HH:mm:ss" (SQLite datetime('now'))
        static member Parse(value: string) =
            let formats = [| "yyyy-MM-ddTHH:mm:ss"; "yyyy-MM-dd HH:mm:ss" |]

            match DateTime.TryParseExact(value, formats, CultureInfo.InvariantCulture, DateTimeStyles.None) with
            | true, dateTime -> DateTimePattern dateTime
            | false, _ ->
                failwith
                    $"Invalid DateTime format: {value}. Expected format: yyyy-MM-ddTHH:mm:ss or yyyy-MM-dd HH:mm:ss"
    ...
    type Money =
        ...
        static member FromAmount(amount: decimal) = Money amount
