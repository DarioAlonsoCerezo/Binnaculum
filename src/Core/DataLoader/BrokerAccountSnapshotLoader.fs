                ...
                |> Seq.map (fun brokerAccount ->
                    async {
                        ...
                        | Some dbSnapshot ->
                            ...
                            let! financialSnapshots = loadFinancialSnapshotsForBrokerAccountSnapshot brokerAccount.Id dbSnapshot.Base.Date |> Async.AwaitTask
                            return Some (dbSnapshot.brokerAccountSnapshotToOverviewSnapshot(financialSnapshots, brokerAccount))
                        | None ->
                            // Create default snapshot for broker account without snapshots
                            let! databaseBrokerAccount = BrokerAccountExtensions.Do.getById brokerAccount.Id |> Async.AwaitTask
                            match databaseBrokerAccount with
                            | Some dbAccount -> 
                                do! BrokerAccountSnapshotManager.handleNewBrokerAccount(dbAccount) |> Async.AwaitTask |> Async.Ignore
                                let! newSnapshot = BrokerAccountSnapshotExtensions.Do.getLatestByBrokerAccountId brokerAccount.Id |> Async.AwaitTask
                                ...
                                | Some dbSnapshot ->
                                    ...
                                    return Some (dbSnapshot.brokerAccountSnapshotToOverviewSnapshot(financialSnapshots, brokerAccount))
                                | None ->
                                    failwithf "Failed to create or retrieve snapshot for broker account %s (ID: %d)" brokerAccount.AccountNumber brokerAccount.Id
                                    return None
                            | None ->
                                failwithf "Failed to retrieve database broker account %s (ID: %d)" brokerAccount.AccountNumber brokerAccount.Id
                                return None
                    })
                |> Async.Parallel
