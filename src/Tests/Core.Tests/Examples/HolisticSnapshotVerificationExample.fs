...
type HolisticSnapshotVerificationExample() =
    ...
    member this.``Example - Holistic BrokerFinancialSnapshot verification``() =
        async {
            ...
            printfn "\n3. Import test data"
            let csvPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "TestData", "Tastytrade_Samples", "TastytradeOptionsTest.csv")
            ...
            let expected: BrokerFinancialSnapshot = {
                ...
                OptionsIncome = 54.37m  // Expected: $54.37 options income (should match)
    ...
    member _.``Example - Compile-time safety demonstration``() =
        ...
        let (allMatch, _) = TestVerifications.verifyBrokerFinancialSnapshot exampleSnapshot exampleSnapshot
