...
type TastytradeImportTests() =
    ...
    let testDataPath = Path.Combine(__SOURCE_DIRECTORY__, "TestData", "Tastytrade_Samples")
    ...
    member this.``TastytradeStatementParser should parse options heavy sample file``() =
        ...
        let optionTransactions = result.Transactions |> List.filter (fun t -> TransactionTypeDetection.isOptionTransaction t.InstrumentType)
    ...
    member this.``TastytradeStatementParser should parse mixed trading sample file``() =
        ...
        let equityTrades = result.Transactions |> List.filter (fun t -> TransactionTypeDetection.isEquityTrade t.InstrumentType)
        let optionTrades = result.Transactions |> List.filter (fun t -> TransactionTypeDetection.isOptionTransaction t.InstrumentType)
        let moneyMovements = result.Transactions |> List.filter (fun t -> match t.TransactionType with MoneyMovement(_) -> true | _ -> false)
    ...
    member this.``TastytradeStatementParser should parse ACAT transfer sample file``() =
        ...
        let acatTransfers = result.Transactions |> List.filter (fun t -> match t.TransactionType with ReceiveDeliver("ACAT") -> true | _ -> false)
        Assert.That(acatTransfers.Length, Is.EqualTo(2))

    [<Test>]
    member this.``TastytradeStatementParser should handle dates with timezones correctly``() =
        let csvContent = """Date,Type,Sub Type,Action,Symbol,Instrument Type,Description,Value,Quantity,Average Price,Commissions,Fees,Multiplier,Root Symbol,Underlying Symbol,Expiration Date,Strike Price,Call or Put,Order #,Currency
2024-05-31T14:42:13+0100,Money Movement,Deposit,,,,Test deposit,1000.00,0,,0.00,0.00,,,,,,,,USD"""
    ...
    member this.``TastytradeStrategyDetector should group transactions by order number``() =
        ...
        let transactions = [
            { Date = DateTime.Now; TransactionType = Trade(BuyToClose, BUY_TO_CLOSE); Symbol = Some "AAPL"; InstrumentType = Some "Equity Option"; Description = "Test 1"; Value = -50m; Quantity = 1m; AveragePrice = Some -50m; Commissions = 1m; Fees = 0.13m; Multiplier = Some 100m; RootSymbol = Some "AAPL"; UnderlyingSymbol = Some "AAPL"; ExpirationDate = Some (DateTime(2024, 5, 31)); StrikePrice = Some 180m; CallOrPut = Some "CALL"; OrderNumber = Some "123456789"; Currency = "USD"; RawCsvLine = ""; LineNumber = 1 }
            { Date = DateTime.Now; TransactionType = Trade(SellToOpen, SELL_TO_OPEN); Symbol = Some "AAPL"; InstrumentType = Some "Equity Option"; Description = "Test 2"; Value = 75m; Quantity = 1m; AveragePrice = Some 75m; Commissions = 1m; Fees = 0.14m; Multiplier = Some 100m; RootSymbol = Some "AAPL"; UnderlyingSymbol = Some "AAPL"; ExpirationDate = Some (DateTime(2024, 6, 14)); StrikePrice = Some 185m; CallOrPut = Some "CALL"; OrderNumber = Some "123456789"; Currency = "USD"; RawCsvLine = ""; LineNumber = 2 }
        ]
    ...
    member this.``TastytradeStrategyDetector should detect calendar spread``() =
        ...
        let transactions = [
            { Date = DateTime.Now; TransactionType = Trade(BuyToClose, BUY_TO_CLOSE); Symbol = Some "AAPL"; InstrumentType = Some "Equity Option"; Description = "Buy to close near expiration"; Value = -50m; Quantity = 1m; AveragePrice = Some -50m; Commissions = 1m; Fees = 0.13m; Multiplier = Some 100m; RootSymbol = Some "AAPL"; UnderlyingSymbol = Some "AAPL"; ExpirationDate = Some (DateTime(2024, 5, 31)); StrikePrice = Some 180m; CallOrPut = Some "CALL"; OrderNumber = Some "123456789"; Currency = "USD"; RawCsvLine = ""; LineNumber = 1 }
            { Date = DateTime.Now; TransactionType = Trade(SellToOpen, SELL_TO_OPEN); Symbol = Some "AAPL"; InstrumentType = Some "Equity Option"; Description = "Sell to open far expiration"; Value = 75m; Quantity = 1m; AveragePrice = Some 75m; Commissions = 1m; Fees = 0.14m; Multiplier = Some 100m; RootSymbol = Some "AAPL"; UnderlyingSymbol = Some "AAPL"; ExpirationDate = Some (DateTime(2024, 6, 14)); StrikePrice = Some 180m; CallOrPut = Some "CALL"; OrderNumber = Some "123456789"; Currency = "USD"; RawCsvLine = ""; LineNumber = 2 }
        ]
    ...
    member this.``TastytradeStrategyDetector should detect single leg``() =
        let transactions = [
            { Date = DateTime.Now; TransactionType = Trade(SellToOpen, SELL_TO_OPEN); Symbol = Some "NVDA"; InstrumentType = Some "Equity Option"; Description = "Single leg trade"; Value = 250m; Quantity = 1m; AveragePrice = Some 250m; Commissions = 1m; Fees = 0.13m; Multiplier = Some 100m; RootSymbol = Some "NVDA"; UnderlyingSymbol = Some "NVDA"; ExpirationDate = Some (DateTime(2024, 5, 31)); StrikePrice = Some 400m; CallOrPut = Some "CALL"; OrderNumber = Some "111222333"; Currency = "USD"; RawCsvLine = ""; LineNumber = 1 }
        ]
...
2024-05-31T14:42:13+0100,Trade,Buy to Close,BUY_TO_CLOSE"""  // Incomplete line
    ...
    member this.``TastytradeStatementParser should handle invalid dates``() =
        let csvContent = """Date,Type,Sub Type,Action,Symbol,Instrument Type,Description,Value,Quantity,Average Price,Commissions,Fees,Multiplier,Root Symbol,Underlying Symbol,Expiration Date,Strike Price,Call or Put,Order #,Currency
INVALID_DATE,Trade,Buy to Close,BUY_TO_CLOSE,AAPL,Equity,Test,-100,1,-100,0,0,,,,,,,123,USD"""
    ...
    member this.``TastytradeStatementParser should parse decimal values correctly``() =
        let csvContent = """Date,Type,Sub Type,Action,Symbol,Instrument Type,Description,Value,Quantity,Average Price,Commissions,Fees,Multiplier,Root Symbol,Underlying Symbol,Expiration Date,Strike Price,Call or Put,Order #,Currency
2024-05-31T14:42:13+0100,Money Movement,Deposit,,,,Test deposit,"1,234.56",0,,0.00,0.00,,,,,,,,USD"""
